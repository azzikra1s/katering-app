...
<div class="min-h-screen flex flex-col">
    ...
    <!-- Navigation -->
    <nav class="bg-white border-b border-gray-200 sticky top-0 z-40" x-data="{ open: false }">
                ...
                <div class="flex items-center gap-4">
                    @auth
                        <div class="relative" x-data="{ dropdown: false }">
                            <button @click="dropdown = !dropdown" @click.outside="dropdown = false"
        ...
        <div x-show="open" x-transition class="md:hidden bg-white border-t border-gray-200">
            @auth
                <div class="flex flex-col px-4 py-3 gap-2">
                    @if(auth()->user()->role === 'customer')
                        <a href="{{ route('customer.dashboard') }}" class="block text-gray-700 hover:bg-gray-50 rounded-lg px-3 py-2">Beranda</a>
                        <a href="{{ route('customer.merchants') }}" class="block text-gray-700 hover:bg-gray-50 rounded-lg px-3 py-2">Cari Katering</a>
                        <a href="{{ route('customer.orders') }}" class="block text-gray-700 hover:bg-gray-50 rounded-lg px-3 py-2">Pesanan</a>
                        <a href="{{ route('profile.edit') }}" class="block text-gray-700 hover:bg-gray-50 rounded-lg px-3 py-2">Profil</a>
                    @else
                        <a href="{{ route('merchant.dashboard') }}" class="block text-gray-700 hover:bg-gray-50 rounded-lg px-3 py-2">Dasbor</a>
                        <a href="{{ route('merchant.menus.index') }}" class="block text-gray-700 hover:bg-gray-50 rounded-lg px-3 py-2">Menu</a>
                        <a href="{{ route('merchant.orders') }}" class="block text-gray-700 hover:bg-gray-50 rounded-lg px-3 py-2">Pesanan</a>
                        <a href="{{ route('merchant.profile') }}" class="block text-gray-700 hover:bg-gray-50 rounded-lg px-3 py-2">Profil</a>
                    @endif
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full text-left px-3 py-2 text-gray-700 hover:bg-red-50 rounded-lg">
                            Keluar
                        </button>
                    </form>
                </div>
            @else
                <div class="flex flex-col px-4 py-3 gap-2">
                    <a href="{{ route('login') }}" class="block text-gray-700 hover:bg-gray-50 rounded-lg px-3 py-2">Masuk</a>
                    <a href="{{ route('register') }}" class="block text-white bg-blue-600 hover:bg-blue-700 rounded-lg text-center px-3 py-2">Daftar</a>
                </div>
            @endauth
        </div>
    </nav>
